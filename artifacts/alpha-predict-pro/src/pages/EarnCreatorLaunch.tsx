...
import { AlertCircle, CheckCircle, Crown, Gift, Play, Rocket, Send, Share2, Sparkles, Trophy, Tv, Video } from "lucide-react";
...
type ContentPlatform = "tiktok" | "instagram" | "youtube" | "x";
type PostType = "post" | "short" | "long";
...
const POST_TYPES: Array<{ id: PostType; label: string; hint: string }> = [
  { id: "short", label: "Short", hint: "Fast clips" },
  { id: "long", label: "Long", hint: "Best rewards" },
...
const REWARD_TIERS = [
  { label: "Valid post", reward: "+50 XP instantly", note: "public link check" },
  { label: "Admin approved", reward: "XP + TC", note: "manual review" },
  { label: "YouTube Shorts", reward: "better view rewards", note: "small GC possible" },
  { label: "YouTube Long-form", reward: "highest multiplier", note: "best for serious creators" },
...
export default function EarnCreatorLaunch() {
  ...
  const creatorStats = useMemo(() => ({
    ...
    vipRefs: submissions.reduce((sum, s) => sum + (s.vipReferrals ?? 0), 0),
  }), [submissions]);
  ...
  return <div className="min-h-screen bg-black px-4 pb-28 pt-4 text-white">
    ...
    {activeTab === "creator" && <div className="space-y-4"><div className="earn-card rounded-3xl p-4 border-[#FFD700]/35"><div className="flex items-center gap-3 mb-3"><Rocket size={24} className="text-[#FFD700]"/><div><div className="font-black text-xl">Creator Missions</div><div className="font-mono text-[10px] text-white/45">Free and VIP users can submit. VIP earns faster.</div></div></div><div className="rounded-2xl border border-[#FFD700]/20 bg-[#FFD700]/8 p-3 font-mono text-[10px] text-[#FFD700]">Put this in your caption: <b>{creatorCode}</b> + mention Koinara.</div></div><div className="grid grid-cols-1 gap-2">{REWARD_TIERS.map((tier,index)=><div key={tier.label} className="earn-card rounded-2xl p-3 flex items-center gap-3"><div className="h-9 w-9 rounded-xl border border-[#FFD700]/25 bg-[#FFD700]/10 flex items-center justify-center font-black text-[#FFD700]">{index+1}</div><div className="flex-1"><div className="font-black">{tier.label}</div><div className="font-mono text-[10px] text-white/40">{tier.note}</div></div><div className="font-mono text-xs font-black text-[#FFD700] text-right">{tier.reward}</div></div>)}</div><div className="earn-card rounded-3xl p-4"><div className="font-black text-lg mb-3">Submit video</div><div className="mb-3 grid grid-cols-4 gap-1.5">{PLATFORMS.map(p=><button key={p.id} onClick={()=>setPlatform(p.id)} className={`rounded-xl py-2 font-mono text-[10px] font-black border ${platform===p.id?"border-[#00F5FF] bg-[#00F5FF]/12 text-[#00F5FF]":"border-white/10 text-white/35"}`}>{p.label}</button>)}</div><div className="mb-3 grid grid-cols-3 gap-1.5">{POST_TYPES.map(p=><button key={p.id} onClick={()=>setPostType(p.id)} className={`rounded-xl py-2 border ${postType===p.id?"border-[#FFD700] bg-[#FFD700]/12 text-[#FFD700]":"border-white/10 text-white/35"}`}><div className="font-mono text-[10px] font-black">{p.label}</div><div className="font-mono text-[8px] opacity-60">{p.hint}</div></button>)}</div><input value={contentUrl} onChange={(e)=>setContentUrl(e.target.value)} placeholder="Paste TikTok / Reel / YouTube link" className="mb-2 w-full rounded-2xl border border-white/12 bg-white/[0.04] px-4 py-3 font-mono text-xs text-white outline-none focus:border-[#00F5FF]/50"/><input value={caption} onChange={(e)=>setCaption(e.target.value)} placeholder="Caption / proof text" className="mb-3 w-full rounded-2xl border border-white/12 bg-white/[0.04] px-4 py-3 font-mono text-xs text-white outline-none focus:border-[#FFD700]/50"/>{feedback&&<div className={`mb-3 rounded-2xl border px-3 py-2 font-mono text-[10px] ${feedback.ok?"border-[#00F5FF]/30 bg-[#00F5FF]/8 text-[#00F5FF]":"border-[#FF4D8D]/30 bg-[#FF4D8D]/8 text-[#FF4D8D]"}`}>{feedback.ok?<CheckCircle size={12} className="inline mr-1"/>:<AlertCircle size={12} className="inline mr-1"/>}{feedback.msg}</div>}<button onClick={handleSubmitContent} disabled={!contentUrl.trim()||submitting} className="w-full rounded-2xl bg-gradient-to-r from-[#FFD700] to-[#FF4D8D] py-3 font-black text-black disabled:opacity-45"><Send size={14} className="inline mr-2"/>{submitting?"Submitting...":"Submit for Review"}</button><p className="mt-2 text-center font-mono text-[9px] text-white/28">Big rewards require admin verification. GC is only for proven views/VIP referrals.</p></div>{submissions.length>0&&<div className="space-y-2"><div className="font-mono text-[10px] text-white/40 tracking-[0.18em] uppercase">Your submissions</div>{submissions.slice(0,6).map(sub=><div key={sub.id} className="earn-card rounded-2xl p-3 flex items-center gap-3"><Video size={18} className="text-[#FFD700]"/><div className="flex-1 min-w-0"><div className="truncate font-mono text-[10px] text-white/60">{sub.url}</div><div className="font-mono text-[9px] text-white/30 capitalize">{sub.platform} · {sub.postType??"post"}</div></div><div className={`rounded-full px-2 py-1 font-mono text-[9px] font-black ${sub.status==="approved"?"bg-[#00F5FF]/10 text-[#00F5FF]":sub.status==="rejected"?"bg-[#FF4D8D]/10 text-[#FF4D8D]":"bg-[#FFD700]/10 text-[#FFD700]"}`}>{sub.status}</div></div>)}</div>}</div>}
  </div>;
}
