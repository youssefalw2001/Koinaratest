...
import { AlertCircle, CheckCircle, Crown, Gift, MessageCircle, Play, Rocket, Send, Share2, ShieldAlert, Sparkles, Trophy, Tv, Video, Wallet } from "lucide-react";
...
export default function EarnCreatorLaunch() {
  ...
  const handleCopyInvite = async () => {
    ...
    if (ok) showFeedback(true, "Invite link copied. Send it to creators, friends, or your bio.");
    else {
      const shareUrl = `https://t.me/share/url?url=${encodeURIComponent(referralLink)}&text=${encodeURIComponent("Join me on Koinara and earn with VIP referrals.")}`;
      window.Telegram?.WebApp?.openTelegramLink?.(shareUrl) ?? window.open(shareUrl, "_blank");
      showFeedback(true, "Opened Telegram share. Copy from there if clipboard is blocked.");
    }
  };
  ...
  const handleWatchAd = async () => {
    ...
    } catch { showFeedback(false, "Ad reward failed. Try again."); }
  };
  ...
  const handleSubmitContent = async () => {
    ...
    try {
      ...
      setContentUrl("");
      showFeedback(true, data?.message ?? "+50 XP submitted. Review pending.", 4500);
      fetchSubmissions();
      queryClient.invalidateQueries({ queryKey: getGetUserQueryKey(user.telegramId) });
    } catch (err) { showFeedback(false, err instanceof Error ? err.message : "Invalid URL or submission failed.", 4500); }
