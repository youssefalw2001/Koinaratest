...
export default function Terminal() {
  ...
  // Price state
  const [price, setCurrentPrice] = useState<number>(0);
  const [prevPrice, setPrevPrice] = useState<number>(0);
  const latestPriceRef = useRef<number>(0);
  const [priceChange, setPriceChange] = useState(0);
  const firstPriceRef = useRef(0);
  ...
  const loadInitialCandles = useCallback(async (symbol: string) => {
    ...
    const candles = [...list].reverse().map((k: any[]) => ({
  ...
  const startPriceFeed = useCallback((symbol: string) => {
    ...
    updateIntervalRef.current = setInterval(async () => {
      try {
        ...
        const data = await res.json();
        const parsed = parseFloat(data?.result?.list?.[0]?.lastPrice);
        if (!Number.isFinite(parsed) || parsed <= 0) return;
        const nextPrice = truncateToTwoDecimals(parsed);
        const now = Math.floor(Date.now() / 1000);
        const minuteStart = Math.floor(now / 60) * 60;
        const openPrice = priceRef.current || nextPrice;

        candleSeriesRef.current?.update({
          time: minuteStart,
          open: openPrice,
          high: Math.max(openPrice, nextPrice),
          low: Math.min(openPrice, nextPrice),
          close: nextPrice,
        });
  ...
  useEffect(() => {
    ...
    startPriceFeed(selectedPair.id);
  ...
  }, [selectedPair.id, startPriceFeed, stopPriceFeed]);
  ...
  useEffect(() => {
    ...
    priceLineRef.current = entryLine;
  }, [activePrediction]);
          ...
          <div className="flex items-end justify-between px-5 mt-4">
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                ...
                <span className="text-[10px] font-mono text-white/20">· Bybit Realtime</span>
              </div>
            </div>
