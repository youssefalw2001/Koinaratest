...
import { ArrowDown, ArrowUp, ChevronDown, Lock, Loader2, Zap } from "lucide-react";
...
const PAIRS = [
  { id: "BTCUSDT", label: "BTC / USDT", coin: "B", seed: 65000 },
  { id: "ETHUSDT", label: "ETH / USDT", coin: "E", seed: 3100 },
  { id: "SOLUSDT", label: "SOL / USDT", coin: "S", seed: 145 },
  { id: "BNBUSDT", label: "BNB / USDT", coin: "N", seed: 580 },
  { id: "XRPUSDT", label: "XRP / USDT", coin: "X", seed: 0.54 },
] as const;
...
async function fetchCandles(symbol: string): Promise<Point[]> {
  const res = await fetch(`https://api.bybit.com/v5/market/kline?category=spot&symbol=${symbol}&interval=1&limit=42`);
  if (!res.ok) return [];
  const data = await res.json();
...
export default function TerminalTradeHotfix({ tradeCap, onTradeResolved }: Props) {
  ...
  const applyPrice = useCallback((raw: number, version: number, state: "live" | "retrying" = "live") => {
    ...
    const display = next === prev ? truncatePrice(synthetic(prev, next)) : next;
    setPreviousPrice(prev);
    setPrice(display);
    latestPriceRef.current = display;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setFeedState("connecting"); setPrice(0); setPreviousPrice(0); setFirstPrice(0); setPoints([]); latestPriceRef.current = 0; realFetchRef.current = 0;
    const boot = async () => {
      try {
        ...
        if (candles.length) {
          ...
          setPrice(last); setPreviousPrice(last); setFirstPrice(last); latestPriceRef.current = last; setFeedState("live");
        }
  ...
  return <div className="min-h-screen pb-24 px-3 pt-2 bg-[#05070d] text-white">
    ...
    <section className="trade-glass rounded-3xl overflow-hidden mb-2"><div className="flex items-center justify-between p-3 pb-1.5"><div className="relative min-w-0"><button onClick={() => setShowPairMenu((v) => !v)} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-2.5 py-2 max-w-[220px]"><div className="h-8 w-8 rounded-full bg-[#FFB000] flex items-center justify-center font-black text-black text-sm">{selectedPair.coin}</div><div className="text-left min-w-0"><div className="flex items-center gap-1.5"><span className="font-black text-white tracking-wide text-sm truncate">{selectedPair.label}</span><span className={`h-2 w-2 rounded-full ${feedState === "live" ? "bg-[#00E676] shadow-[0_0_10px_rgba(0,230,118,.85)]" : "bg-[#FFD700] shadow-[0_0_10px_rgba(255,215,0,.7)]"}`} /></div><div className="font-mono text-[10px] text-white/65 truncate">{price > 0 ? formatPrice(price) : "Connecting"} <span className={trendUp ? "text-[#00E676]" : "text-[#FF4D6D]"}>{priceChange >= 0 ? "+" : ""}{priceChange.toFixed(2)}%</span></div></div><ChevronDown size={13} className="text-white/40" /></button><AnimatePresence>{showPairMenu && <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="absolute z-30 mt-2 w-48 rounded-2xl border border-white/10 bg-[#101522] p-2 shadow-2xl">{PAIRS.map((pair, index) => <button key={pair.id} onClick={() => { setPairIndex(index); setShowPairMenu(false); setResult(null); setTradeError(null); }} className={`w-full rounded-xl px-3 py-2 text-left font-mono text-xs font-black ${index === pairIndex ? "bg-[#FFD700] text-black" : "text-white/55 hover:bg-white/8"}`}>{pair.label}</button>)}</motion.div>}</AnimatePresence></div><div className="flex items-center gap-2"><div className="rounded-xl border border-white/10 bg-white/[0.04] px-2 py-1.5 font-mono text-[10px] text-white/65">1m</div><div className="rounded-xl border border-white/10 bg-white/[0.04] px-2 py-1.5 font-mono text-[10px] text-white/45">{feedState === "live" ? "LIVE" : "SYNC"}</div></div></div><div className="px-3 flex items-end justify-between gap-2"><div className={trendUp ? "text-[#00E676]" : "text-[#FF4D6D]"}><div className="text-[30px] leading-none font-black tracking-tight tabular-nums">{price > 0 ? `$${formatPrice(price)}` : "Connecting..."}</div>{activePrediction && <div className="font-mono text-[10px] text-[#FFD700] mt-1">Entry {formatPrice(activePrediction.entryPrice)} · {countdown}s</div>}</div><div className="text-right font-mono text-[10px] text-white/45"><div className="text-[#00E676]">Bulls {sentiment.toFixed(0)}%</div><div className="text-[#FF4D6D]">Bears {(100 - sentiment).toFixed(0)}%</div></div></div><div className="h-[198px] px-3 mt-1 flex items-center justify-center"><svg viewBox="0 0 340 178" className="h-full w-full overflow-visible"><defs><linearGradient id="koinaraTradeLine" x1="0" x2="1"><stop offset="0%" stopColor="#00E676"/><stop offset="100%" stopColor="#4DA3FF"/></linearGradient></defs><path d={pathFor(points)} fill="none" stroke="url(#koinaraTradeLine)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" filter="drop-shadow(0 0 8px rgba(77,163,255,.45))" />{points.length === 0 && <text x="170" y="92" textAnchor="middle" fill="rgba(255,255,255,.35)" fontSize="14">Connecting...</text>}{activePrediction && <line x1="0" x2="340" y1="88" y2="88" stroke="#FFD700" strokeWidth="1.5" strokeDasharray="5 5" />}</svg></div><div className="px-3 pb-3"><div className="h-1.5 rounded-full bg-white/8 overflow-hidden flex"><div className="h-full bg-[#00E676]" style={{ width: `${sentiment}%` }} /><div className="h-full bg-[#FF1744]" style={{ width: `${100 - sentiment}%` }} /></div></div></section>
