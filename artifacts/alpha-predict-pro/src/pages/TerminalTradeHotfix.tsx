...
type FeedState = "connecting" | "live" | "simulated" | "retrying";
...
async function fetchLivePrice(symbol: string): Promise<{ price: number; isLive: boolean } | null> {
  ...
  return { price: truncatePrice(raw), isLive: data?.source === "live" };
}
...
export default function TerminalTradeHotfix({ tradeCap, onTradeResolved }: Props) {
  ...
  const liveReady = (feedState === "live" || feedState === "simulated") && price > 0;

  const applyPrice = useCallback((raw: number, state: "live" | "simulated" = "live") => {
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      if (cancelled) return;
      if (live) applyPrice(live.price, live.isLive ? "live" : "simulated"); else setFeedState("retrying");
    };
    boot();
  ...
  const handlePredict = useCallback(async (direction: "long" | "short") => {
    ...
    if (!liveReady) { setTradeError("Market price feed is unavailable. Please wait for the feed to reconnect."); return; }
    setTradeError(null);
  ...
  return <div className="min-h-screen pb-24 px-3 pt-2 bg-[#05070d] text-white">
    ...
    {feedState === "simulated" && <div className="mb-2 rounded-2xl border border-[#FFD700]/30 bg-[#FFD700]/8 px-3 py-2 font-mono text-[11px] text-[#FFD700] flex items-center gap-2"><span className="inline-block w-1.5 h-1.5 rounded-full bg-[#FFD700] animate-pulse" />Demo mode — prices sourced from backend reference feed. Trades are live.</div>}
    {(feedState === "connecting" || feedState === "retrying") && <div className="mb-2 rounded-2xl border border-[#FFD700]/30 bg-[#FFD700]/8 px-3 py-2 font-mono text-[11px] text-[#FFD700]">Waiting for market price feed. Trades are paused until the feed reconnects.</div>}
    ...
    <section className="trade-glass rounded-3xl overflow-hidden mb-2">
      <div className="flex items-center justify-between p-3 pb-1.5">
        ...
        <div className="flex items-center gap-2"><div className="rounded-xl border border-white/10 bg-white/[0.04] px-2 py-1.5 font-mono text-[10px] text-white/65">1m</div><div className={`rounded-xl border px-2 py-1.5 font-mono text-[10px] ${feedState === "live" ? "border-[#00E676]/25 bg-[#00E676]/8 text-[#00E676]" : feedState === "simulated" ? "border-[#FFD700]/25 bg-[#FFD700]/8 text-[#FFD700]" : "border-[#FF4D6D]/25 bg-[#FF4D6D]/8 text-[#FF4D6D]"}`}>{feedState === "live" ? "LIVE" : feedState === "simulated" ? "DEMO" : "WAIT"}</div></div>
      </div>
