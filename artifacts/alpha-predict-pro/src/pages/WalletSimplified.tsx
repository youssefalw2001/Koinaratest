...
export default function WalletSimplified() {
  ...
  const buyVip = async () => {
    ...
    try {
      ...
      setMessage((err as { message?: string })?.message ?? "VIP activation failed.");
    } finally {
      setTonPending(false);
    }
  };
  ...
  const verifyIdentity = async () => {
    ...
    try {
      await tonConnectUI.sendTransaction({
        validUntil: Math.floor(Date.now() / 1000) + 600,
        messages: [{ address: KOINARA_TON_WALLET, amount: TON_VERIFY_AMOUNT, payload: commentPayload(`KNR-VERIFY-${user.telegramId}`) }],
      });
      await new Promise((r) => setTimeout(r, 5000));
  ...
  const submitWithdrawal = async () => {
    ...
    try {
      const result = await requestWithdrawal(
        { telegramId: user.telegramId, gcAmount, usdtWallet },
        { headers: { "Idempotency-Key": idempotencyKey(`withdraw-${source}`, [user.telegramId, gcAmount, usdtWallet]) } },
      );
      ...
      setMessage((err as { message?: string })?.message ?? "Withdrawal failed.");
    } finally {
  ...
  const chooseSource = (nextSource: WithdrawSource) => {
    ...
    setMessage(nextSource === "creator"
      ? "Creator and referral earnings are paid in GC and withdraw the same way as gameplay GC."
      : "Gameplay earnings selected. Use your GC balance for withdrawal.");
  };
