...
const TC_PACKS: TcPack[] = [
  { id: "micro", name: "Micro Refill", price: "$0.99", tc: 7000, desc: "Quick refill for another session.", badge: "FAST", tone: "#4DA3FF", icon: Zap },
  { id: "starter", name: "Starter Pack", price: "$2.99", tc: 30000, desc: "Best first top-up for new players.", badge: "BEST START", tone: "#B65CFF", icon: Star },
  { id: "pro", name: "Pro Pack", price: "$9.99", tc: 150000, desc: "Built for daily active traders.", badge: "POPULAR", tone: "#FFD700", icon: Rocket },
  { id: "whale", name: "Whale Pack", price: "$49.99", tc: 1000000, desc: "Maximum balance for high-volume users.", badge: "MAX", tone: "#00F5A0", icon: Flame },
];
    ...
    <div className="min-h-screen px-3 pt-3 pb-28 text-white bg-[#05070d]">
      ...
      <section className="grid grid-cols-[1fr_auto] gap-2 items-center mb-3">
        <div className="flex items-center gap-3">
          <div className="h-12 w-12 rounded-full border border-[#FFD700]/45 bg-[#FFD700]/10 flex items-center justify-center shadow-[0_0_28px_rgba(255,215,0,.18)]"><span className="text-xl font-black text-[#FFD700]">K</span></div>
          <div><h1 className="text-2xl font-black tracking-[0.22em] text-[#FFD700] leading-none">KOINARA</h1><p className="font-mono text-[10px] tracking-[0.26em] text-white/45 mt-1">SHOP TERMINAL</p></div>
        </div>
      ...
      <section className="grid grid-cols-[1fr_1fr] gap-2 mb-3">
        <div className="shop-glass rounded-2xl p-3"><div className="font-mono text-2xl font-black text-[#8BC3FF] tabular-nums">{(user?.tradeCredits ?? 0).toLocaleString()}</div><div className="font-mono text-[10px] text-white/45">Trade Credits</div></div>
        <div className="shop-glass shop-gold rounded-2xl p-3"><div className="font-mono text-2xl font-black text-[#FFD700] tabular-nums">{(user?.goldCoins ?? 0).toLocaleString()}</div><div className="font-mono text-[10px] text-white/45">Game Coins</div></div>
      </section>
      ...
      <section className="shop-glass shop-purple rounded-3xl p-4 mb-3 overflow-hidden relative">
        ...
        <div className="grid grid-cols-[94px_1fr] gap-4 relative z-10">
          <div className="h-28 rounded-[28px] border border-[#B65CFF]/45 bg-gradient-to-br from-[#B65CFF]/35 to-[#19082f] flex flex-col items-center justify-center shadow-[0_0_45px_rgba(182,92,255,.28)]"><Crown size={38} className="text-[#E7C4FF] drop-shadow-[0_0_18px_rgba(231,196,255,.8)]" /><div className="font-black text-2xl text-[#E7C4FF] mt-1">VIP</div></div>
          <div><div className="font-mono text-[10px] tracking-[0.2em] uppercase text-[#D9A8FF] mb-1">VIP Membership</div><h2 className="text-2xl font-black leading-tight">Trade. Earn. Keep More.</h2><div className="text-2xl font-black text-[#D9A8FF] mt-2">$5.99 <span className="text-sm text-white/55 font-mono">/ month</span></div><Link href="/wallet" className="mt-3 h-11 rounded-2xl bg-gradient-to-r from-[#8A35FF] to-[#E26BFF] flex items-center justify-center gap-2 font-black shadow-[0_0_30px_rgba(182,92,255,.35)]"><Crown size={16} /> Activate VIP</Link></div>
        </div>
      </section>

      <section className="shop-glass shop-cyan rounded-3xl p-4 mb-3">
        <div className="flex items-center gap-3"><Users size={34} className="text-[#00F5FF]" /><div><div className="font-mono text-[10px] tracking-[0.18em] uppercase text-[#00F5FF]">Earn more together</div><div className="text-lg font-black"><span className="text-[#00F5FF]">20%</span> direct VIP + <span className="text-[#00F5FF]">5%</span> level 2</div><p className="font-mono text-[10px] text-white/48 mt-1">Invite 1 VIP to waive first withdrawal verification.</p></div></div>
      </section>
      ...
      <section className="rounded-3xl border border-[#B65CFF]/35 bg-gradient-to-r from-[#B65CFF]/14 via-[#0B1020] to-[#FFD700]/10 p-3 mb-3 flex items-center justify-between">
        <div className="flex items-center gap-3"><Gift size={25} className="text-[#D9A8FF]" /><div><div className="font-mono text-[10px] text-[#D9A8FF] tracking-[0.18em] uppercase">Limited time offer</div><div className="font-black">Power up your trading edge!</div><div className="font-mono text-[10px] text-white/45">TC packs and power-ups are ready.</div></div></div>
        <div className="rounded-2xl border border-[#FFD700]/30 bg-[#FFD700]/8 px-3 py-2 text-right"><div className="font-mono text-[9px] text-white/45">Ends in</div><div className="font-mono text-sm font-black text-[#FFD700]">15h</div></div>
      </section>

      <section className="grid grid-cols-4 gap-1.5 mb-3">
        {([
          ["tc", Coins, "TC"],
          ["powerups", Zap, "Power"],
          ["vip", Crown, "VIP"],
          ...
          <button key={tab} onClick={() => setActiveTab(tab)} className={`rounded-2xl border py-2.5 font-black text-xs flex items-center justify-center gap-1.5 ${activeTab === tab ? "border-[#4DA3FF] bg-[#4DA3FF]/14 text-white shadow-[0_0_22px_rgba(77,163,255,.18)]" : "border-white/10 bg-white/[0.03] text-white/42"}`}><Icon size={15} />{label}</button>
        ))}
      </section>
      ...
      {activeTab === "tc" && (
        <section className="space-y-2">
          {TC_PACKS.map((pack) => {
            ...
            return (
              <motion.div key={pack.id} layout className="shop-glass rounded-3xl p-3 relative overflow-hidden" style={{ borderColor: `${pack.tone}55` }}>
                {pack.badge && <div className="absolute right-0 top-0 rounded-bl-2xl px-3 py-1 text-[9px] font-black" style={{ background: `${pack.tone}CC`, color: pack.tone === "#FFD700" ? "#090909" : "white" }}>{pack.badge}</div>}
                <div className="flex items-center gap-3">
                  <PremiumIcon tone={pack.tone}><Icon size={26} style={{ color: pack.tone }} /></PremiumIcon>
                  <div className="flex-1 min-w-0"><div className="text-base font-black">{pack.name}</div><div className="font-mono text-2xl font-black" style={{ color: pack.tone }}>{pack.tc.toLocaleString()} TC</div><div className="font-mono text-[10px] text-white/45">{pack.desc}</div></div>
                  <div className="text-right"><div className="font-mono text-lg font-black text-white">{pack.price}</div><Link href="/wallet" className="mt-2 rounded-xl border border-[#FFD700]/30 bg-[#FFD700]/10 px-3 py-2 font-mono text-[10px] font-black text-[#FFD700] flex items-center gap-1"><Wallet size={12} />Fund</Link></div>
                </div>
              </motion.div>
            );
          })}
          <div className="rounded-2xl border border-[#FFD700]/20 bg-[#FFD700]/8 p-3 font-mono text-[10px] text-[#FFD700]/85 flex items-start gap-2"><Sparkles size={13} className="shrink-0 mt-0.5" />TC pack cards are now visible. Payment wiring can connect to the existing funding flow when the backend pack endpoint is ready.</div>
        </section>
      )}
      ...
      {(activeTab === "powerups" || activeTab === "boosts") && (
          ...
          {cards.map((card) => {
            ...
            return (
              <motion.div key={card.id} layout className="shop-glass rounded-2xl p-3 relative overflow-hidden" style={{ borderColor: `${card.tone}44` }}>
                ...
                <PremiumIcon tone={card.tone}><Icon size={26} style={{ color: card.tone }} /></PremiumIcon>
                ...
                <div className="mt-3 flex items-center justify-between gap-2">
                  <div><div className="font-mono text-[10px] text-white/35">{card.uses}</div><div className="font-mono text-sm font-black text-[#FFD700]">{card.cost > 0 ? `${card.cost.toLocaleString()} GC` : "TON"}</div></div>
                  <button onClick={() => handleBuy(card)} disabled={locked || (!canAfford && card.cost > 0) || purchaseMutation.isPending} className="rounded-xl border border-[#FFD700]/35 bg-[#FFD700]/10 px-3 py-2 font-black text-[#FFD700] disabled:opacity-35">{locked ? <Lock size={15} /> : confirm ? "Confirm" : "Buy"}</button>
                </div>
              </motion.div>
            );
          })}
