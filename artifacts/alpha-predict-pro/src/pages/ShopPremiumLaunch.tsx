  ...
  return (
    <div className="min-h-screen px-3 pt-3 pb-28 text-white bg-[#05070d]">
      ...
      {activeTab === "tc" && <section className="space-y-2">
        <motion.div layout className="shop-glass shop-gold rounded-3xl p-3 relative overflow-hidden"><div className="absolute right-0 top-0 rounded-bl-2xl bg-[#FFD700] px-3 py-1 text-[9px] font-black text-black">LIMIT BOOST</div><div className="flex items-center gap-3"><PremiumIcon tone={TRADE_OVERTIME.tone}><Zap size={26} className="text-[#FFD700]" /></PremiumIcon><div className="flex-1 min-w-0"><div className="text-base font-black">{TRADE_OVERTIME.name}</div><div className="font-mono text-2xl font-black text-[#FFD700]">+{TRADE_OVERTIME.boostGc.toLocaleString()} GC CAP</div><div className="font-mono text-[10px] text-white/45">{TRADE_OVERTIME.desc} One per UTC day.</div></div><div className="text-right"><div className="font-mono text-lg font-black text-white">{TRADE_OVERTIME.price}</div><button onClick={handleBuyOvertime} disabled={buyingOvertime || !!buyingPack} className="mt-2 rounded-xl border border-[#FFD700]/30 bg-[#FFD700]/10 px-3 py-2 font-mono text-[10px] font-black text-[#FFD700] flex items-center gap-1 disabled:opacity-40"><Wallet size={12} />{buyingOvertime ? "Verifying" : "Buy"}</button></div></div></motion.div>
