...
export default function QuickTradeArenaClean() {
  ...
  useEffect(() => { const t = window.setInterval(() => void loadPrice(), pendingTrade ? 3000 : 8000); return () => window.clearInterval(t); }, [loadPrice, pendingTrade?.id]);
  ...
  const resolveTrade = async () => {
    ...
    try {
      ...
      setLastResult(data);
      setRecentTrades((prev) => [data, ...prev.filter((row) => row.id !== data.id)].slice(0, 12));
      setNotice(data.status === "won" ? `Verified win: +${Number(data.payout ?? 0).toLocaleString()} Play Coins.` : "Verified result: trade lost.");
      await refreshUser();
      await loadTrades();
      await loadPrice();
    } catch (err) {
  ...
  return <div className="min-h-screen bg-[#05070d] px-3 pb-28 pt-3 text-white">
    ...
    {tab === "trade" && <>
      {!pendingTrade && <section className="arena-card mb-3 rounded-3xl p-4">
        ...
        <div className="mb-3 grid grid-cols-2 gap-2"><div className="rounded-2xl border border-[#FFD700]/18 bg-[#FFD700]/7 p-3"><div className="font-mono text-[9px] text-white/35">Correct prediction</div><div className="font-black text-[#FFD700]">+{winGc} coins</div></div><div className="rounded-2xl border border-[#4DA3FF]/18 bg-[#4DA3FF]/7 p-3"><div className="font-mono text-[9px] text-white/35">Daily cap</div><div className="font-black text-[#8BC3FF]">{dailyCap.toLocaleString()} coins</div></div></div>
      ...
      {lastResult && !pendingTrade && <section className={`mb-3 rounded-3xl border p-4 ${lastResult.status === "won" ? "border-[#00F5A0]/35 bg-[#00F5A0]/10 text-[#00F5A0]" : "border-[#FF4D6D]/35 bg-[#FF4D6D]/10 text-[#FF8FA3]"}`}><div className="flex items-center gap-2"><CheckCircle size={18}/><h2 className="text-xl font-black">{lastResult.status === "won" ? "Trade Won" : "Trade Lost"}</h2><span className="ml-auto font-mono text-xs">verified</span></div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><div className="font-mono text-[9px] text-white/40">Entry</div><div className="font-black text-white">{formatPrice(lastResult.entryPrice)}</div></div><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><div className="font-mono text-[9px] text-white/40">Exit</div><div className="font-black text-white">{formatPrice(lastResult.exitPrice)}</div></div><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><div className="font-mono text-[9px] text-white/40">Move</div><div className={Number(resultMove ?? 0) >= 0 ? "font-black text-[#00F5A0]" : "font-black text-[#FF8FA3]"}>{formatMove(resultMove)}</div></div><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><div className="font-mono text-[9px] text-white/40">Result</div><div className="font-black text-white">{lastResult.status === "won" ? `+${Number(lastResult.payout ?? 0).toLocaleString()} Play Coins` : `-${lastResult.amount.toLocaleString()} TC`}</div></div></div><button onClick={() => setLastResult(null)} className="mt-3 w-full rounded-2xl border border-white/10 bg-white/[0.04] py-2.5 font-mono text-xs font-black text-white/70">Start another trade</button></section>}
    </>}
    ...
    {tab === "history" && <section className="arena-card mb-3 rounded-3xl p-4"><div className="mb-3 flex items-center gap-2"><Trophy size={17} className="text-[#FFD700]"/><h2 className="font-black">Recent Quick Trades</h2></div><div className="space-y-2">{recentTrades.length === 0 && <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4 text-center font-mono text-xs text-white/35">No Quick Trades yet.</div>}{recentTrades.map((row) => { const move = movement(row); return <div key={row.id} className="rounded-2xl border border-white/10 bg-white/[0.025] p-3"><div className="flex items-center justify-between"><div><div className="font-black">{dirLabel(row.direction)}</div><div className="font-mono text-[10px] text-white/35">{row.amount} TC · {row.duration}s · {timeAgo(row.resolvedAt ?? row.createdAt)}</div><div className="mt-1 font-mono text-[9px] text-white/30">{formatPrice(row.entryPrice)} → {formatPrice(row.exitPrice)} · {formatMove(move)}</div></div><div className={`rounded-xl border px-3 py-2 text-right ${row.status === "won" ? "border-[#00F5A0]/25 bg-[#00F5A0]/8 text-[#00F5A0]" : row.status === "lost" ? "border-[#FF4D6D]/25 bg-[#FF4D6D]/8 text-[#FF8FA3]" : "border-[#FFD700]/25 bg-[#FFD700]/8 text-[#FFD700]"}`}><div className="font-mono text-[9px] uppercase">{row.status}</div><div className="font-black">{row.status === "won" ? `+${Number(row.payout ?? 0).toLocaleString()} Play Coins` : row.status === "lost" ? `-${row.amount} TC` : "Live"}</div></div></div></div>; })}</div></section>}

    <section className="rounded-3xl border border-[#4DA3FF]/20 bg-[#4DA3FF]/7 p-3 font-mono text-[10px] leading-relaxed text-white/45"><Shield size={13} className="mr-1 inline text-[#8BC3FF]"/>Quick Trade is a prediction game. Official entry and exit prices are locked server-side. TC is never withdrawable; only capped Play Coins can be earned.</section>
  </div>;
}
