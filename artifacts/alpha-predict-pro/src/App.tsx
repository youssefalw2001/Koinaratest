...
// Pages
import Arena from "./pages/QuickTradeArenaClean";
      ...
      {visible && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-end justify-center bg-black/85" onClick={dismiss}>
          <motion.div initial={{ y: 300 }} animate={{ y: 0 }} exit={{ y: 300 }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="w-full max-w-[420px] p-6 pb-8 rounded-t-3xl border-t-2 border-[#f5c518]" style={{ background: "linear-gradient(180deg, #0a0800 0%, #000000 100%)", boxShadow: "0 -25px 80px rgba(245,197,24,0.35)" }} onClick={e => e.stopPropagation()}>
            <div className="flex flex-col items-center text-center"><div className="w-16 h-16 rounded-full flex items-center justify-center mb-3 border-2 border-[#f5c518]" style={{ boxShadow: "0 0 30px rgba(245,197,24,0.5)", background: "rgba(245,197,24,0.1)" }}><Crown size={32} className="text-[#f5c518] drop-shadow-[0_0_15px_#f5c518]" /></div><div className="font-mono text-2xl font-black text-[#f5c518] mb-1">Unlock Koinara VIP</div><div className="font-mono text-xs text-white/55 mb-4">Paid VIP only. No free trials. Get higher caps, bigger Battle stakes, and Creator Pass included.</div><div className="w-full space-y-2 mb-5">{["Higher daily play rewards cap for Battle","Higher daily play rewards cap for Mines","Lower withdrawal requirement + faster cashout path","Creator Pass included — referral commissions activated automatically","Monthly renewal rewards from active VIP referrals"].map(perk => <div key={perk} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[#f5c518]/20 bg-[#f5c518]/5"><CheckCircle size={12} className="text-[#f5c518] shrink-0" /><span className="font-mono text-xs text-white text-left">{perk}</span></div>)}</div><button onClick={handleGoVip} className="w-full py-4 rounded-2xl font-mono text-base font-black text-black mb-3" style={{ background: "linear-gradient(90deg, #f5c518, #ff9900)", boxShadow: "0 0 25px rgba(245,197,24,0.5)" }}>PURCHASE NOW</button><button onClick={dismiss} className="font-mono text-xs text-white/30 hover:text-white/50 transition-colors">Not now</button></div>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
...
function RouteAnalytics() {
  const [location] = useLocation();
  const { user } = useTelegram();
  useEffect(() => { trackEvent("page_view", { telegramId: user?.telegramId ?? null, route: location, metadata: { path: location } }); }, [location, user?.telegramId]);
  return null;
}
  ...
  return (
    <Layout>
      ...
      <Switch>
        ...
        <Route path="/dice" component={() => <Bounded><Dice /></Bounded>} />
