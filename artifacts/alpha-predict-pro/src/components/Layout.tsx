  ...
  return (
    <div className="flex flex-col min-h-screen max-w-[420px] mx-auto text-white bg-[#050508]" dir={isArabic ? "rtl" : "ltr"}>
      ...
      <header className="sticky top-0 z-40 border-b border-white/[0.05] premium-glass">
        <div className="flex items-center justify-between px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span
                className="font-black text-[13px] tracking-[0.35em] gold-text-gradient uppercase leading-none"
              >
                KOINARA
              </span>
              <span className="text-[7px] text-white/30 tracking-[0.4em] uppercase mt-1 font-bold">
                Alpha Terminal
              </span>
            </div>
          </div>
        ...
        {user && (
          <div className="px-5 pb-3 flex items-center gap-2">
            <div className="inline-flex items-center gap-1.5 rounded-lg border border-white/[0.08] bg-white/[0.03] px-2.5 py-1.5">
              ...
              <span className="font-mono text-[11px] font-bold text-[#8BC3FF] tabular-nums">
            ...
            <div
              id="gc-balance-pill"
              className="inline-flex items-center gap-1.5 rounded-lg border border-[#FFD700]/20 bg-[#FFD700]/5 px-2.5 py-1.5"
            >
              <div className="w-1.5 h-1.5 rounded-full bg-[#FFD700] shadow-[0_0_8px_#FFD700]" />
              <span className="font-mono text-[11px] font-bold text-[#FFD700] tabular-nums">
            ...
            {vip && (
              <div
                className="ml-auto inline-flex items-center gap-1.5 rounded-lg border border-[#FFD700]/30 bg-[#FFD700]/10 px-2.5 py-1.5"
                style={{ animation: "vip-pulse 4s ease-in-out infinite" }}
              >
        ...
        <div
          className="relative overflow-hidden border-t border-white/[0.03]"
          style={{ height: 26, background: "rgba(0,0,0,0.2)" }}
        >
          <div className="absolute left-0 top-0 flex items-center h-full whitespace-nowrap" style={{ animation: "withdraw-ticker 40s linear infinite" }}>
            {[...tickerItems, ...tickerItems].map((item, idx) => (
              <span key={`${item}-${idx}`} className="inline-flex items-center gap-2 px-6">
                <Sparkles size={10} className="text-[#FFD700]/40" />
                <span className="font-mono text-[9px] text-white/40 font-medium tracking-tight">{item}</span>
              </span>
            ))}
          </div>
        </div>
      ...
      {hasPaidVip && paidVipCountdown && (
        <div className="px-5 py-2 border-b border-[#FFD700]/10 bg-[#FFD700]/[0.02]">
          <div className="flex items-center gap-2">
            <Crown size={12} className="text-[#FFD700]/60" />
            <span className="font-mono text-[10px] text-[#FFD700]/80 font-bold">PREMIUM ACTIVE</span>
            <span className="font-mono text-[10px] text-white/30 ml-auto">{paidVipCountdown}</span>
          </div>
        </div>
      )}
      ...
      {trialCountdown && (
        ...
        >
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            ...
            <span className="font-mono text-[10px] text-white/40 tabular-nums">{trialCountdown}</span>
          </div>
          <Link href="/wallet">
            <span className="font-mono text-[9px] text-[#FFD700] border border-[#FFD700]/30 px-2 py-0.5 rounded-md font-bold hover:bg-[#FFD700]/10 transition-colors">UPGRADE</span>
          </Link>
        </div>
      )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-[420px] mx-auto z-50 border-t border-white/[0.05] premium-glass px-2">
        <div className="flex justify-around items-center h-20">
          {tabs.map(({ path, icon: Icon, labelKey }) => {
            const active = location === path || (path !== "/" && location.startsWith(path));
            return (
              <Link key={path} href={path} className="relative group">
                <div className={`flex flex-col items-center py-2 px-3 gap-1.5 transition-all duration-300 ${active ? "text-[#FFD700]" : "text-white/30 hover:text-white/50"}`}>
                  <div className="relative">
                    {active && (
                      <motion.div 
                        layoutId="nav-glow"
                        className="absolute -inset-2 bg-[#FFD700]/10 blur-md rounded-full"
                      />
                    )}
                    <Icon size={20} className={`relative z-10 ${active ? "drop-shadow-[0_0_10px_rgba(255,215,0,0.5)]" : ""}`} strokeWidth={active ? 2.5 : 2} />
                  </div>
                  <span className={`text-[9px] font-black tracking-[0.15em] uppercase relative z-10 ${active ? "opacity-100" : "opacity-60"}`}>
                    {t(labelKey)}
                  </span>
                  {active && (
                    <motion.div 
