...
const GEM_CATALOG = {
  ...
  // ── Safe Battle in-game power-ups (paid with GC or Stars) ────────────────
  ...
  // ── Mines Power-ups (paid with Stars in this launch; TON direct grant blocked) ──
...
router.post("/gems/purchase", async (req, res): Promise<void> => {
  ...
  if (catalog.category === "mines" && catalog.tonCost > 0) {
    // Critical safety fix: Mines power-ups used to be granted immediately after
    // the frontend clicked the price. They now require a verified Stars webhook
    // credit path. Never trust a client-side click for TON/Stars paid items.
    res.status(402).json({ error: "Mines power-ups require a completed Stars payment." });
    return;
  }
