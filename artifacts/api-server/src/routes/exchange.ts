...
function overtimeMemo(telegramId: string, passId: OvertimePass["id"]): string {
  return `KNR-OVERTIME-${passId}-${telegramId}`;
}
...
router.get("/exchange/tc-packs", (_req, res): void => {
  res.json({
    packs: TC_PACKS.map((p) => ({ id: p.id, label: p.label, priceTon: p.priceTonLabel, priceTonNano: p.priceTonNano.toString(), priceUsd: p.priceUsdLabel, tcAwarded: p.tcAwarded, bonusPct: p.bonusPct })),
  });
});
...
router.get("/exchange/overtime-passes", (_req, res): void => {
  res.json({
    passes: [{
      id: TRADE_OVERTIME_PASS.id,
      label: TRADE_OVERTIME_PASS.label,
      priceTon: TRADE_OVERTIME_PASS.priceTonLabel,
      priceTonNano: TRADE_OVERTIME_PASS.priceTonNano.toString(),
      priceUsd: TRADE_OVERTIME_PASS.priceUsdLabel,
      boostGc: TRADE_OVERTIME_PASS.boostGc,
      maxPerDay: TRADE_OVERTIME_PASS.maxPerDay,
      expires: "UTC daily reset",
    }],
  });
});
...
async function verifyTonPayment(senderAddress: string, expectedNano: bigint, expectedMemo: string): Promise<{ ok: boolean; err?: string; txHash?: string; configErr?: boolean }> {
  ...
  if (!walletEnv) {
    logger.error("[EXCHANGE] KOINARA_TON_WALLET is not set — TON payment processing disabled");
    return { ok: false, err: "TON payment processing is not configured. Please contact support.", configErr: true };
  }
...
router.post("/exchange/tc-pack/purchase", exchangeRateLimiter, async (req, res): Promise<void> => {
  ...
  if (!verification.ok) {
    res.status(verification.configErr ? 503 : 400).json({ error: verification.err ?? "TON payment verification failed.", requiredMemo: expectedMemo });
    return;
  }
  ...
  try {
    const result = await db.transaction(async (tx) => {
      try {
        await tx.insert(tcPackTxHashesTable).values({ txHash, telegramId, pack: pack.id, tcAwarded: pack.tcAwarded });
      } catch (err) {
        const msg = err instanceof Error ? err.message : "";
        if (msg.toLowerCase().includes("unique") || msg.toLowerCase().includes("duplicate")) throw new Error("TX_ALREADY_USED");
        throw err;
      }
    ...
    if (msg === "TX_ALREADY_USED") {
      res.status(409).json({ error: "This TON payment has already been credited. If your TC balance looks wrong, contact support." });
      return;
    }
...
router.post("/exchange/overtime-pass/purchase", exchangeRateLimiter, async (req, res): Promise<void> => {
  ...
  if (alreadyBoostedToday) {
    res.status(409).json({ error: "Trade Overtime Pass already used today. It resets at UTC daily reset." });
    return;
  }
  ...
  try {
    const result = await db.transaction(async (tx) => {
      try {
        await tx.insert(tcPackTxHashesTable).values({ txHash, telegramId, pack: TRADE_OVERTIME_PASS.id, tcAwarded: 0 });
      } catch (err) {
        const msg = err instanceof Error ? err.message : "";
        if (msg.toLowerCase().includes("unique") || msg.toLowerCase().includes("duplicate")) throw new Error("TX_ALREADY_USED");
        throw err;
    ...
    const msg = err instanceof Error ? err.message : "UNKNOWN";
    if (msg === "TX_ALREADY_USED") {
      res.status(409).json({ error: "This TON payment has already been credited. If your cap boost looks wrong, contact support." });
      return;
    }
